...
public partial class SubmittedAssignment : System.Web.UI.Page
{
    ...
    public DataSet Bind()
    {
        SqlConnection Zcon = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionStringAPUASSIGNMENTSYS"].ConnectionString);
        Zcon.Open();
        SqlCommand Zcmd = new SqlCommand("SELECT * FROM  All_Assignment INNER JOIN Assignment_Datas ON All_Assignment.Id = Assignment_Datas.Id WHERE St_TP ='" + TextBox2.Text + "'and St_Name='"+TextBox1.Text+"' and St_Intake='"+TextBox3.Text+"'", Zcon);
